...
class LandView(Drawable, MapTools):
    ...
    def set_world(self, world_size, Point position, background):
    ...
    def move_position(self, Point vector):
        "перемещаем камеру"
        self.surface.set_camera_position(self.surface.position + vector)
        
    def insert(self, list tiles, list observed):
